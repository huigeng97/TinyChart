...
public class Frontend {
	 ...
	 */
	public void runBaseMode(String[]selectedGenres, String[]selectedRatings, Backend backend) {
		...
		while (userInput.equals("x")) {
			List<MovieInterface> currThree=backend.getThreeMovies(0);
			//TODO: if I just print the titles or more info about the movie? 
			for (int i=0; i<currThree.size();i++) {
				System.out.print(currThree.get(i).getTitle()+" ");
			}//prints first three movies 
			...
			currThree=backend.getThreeMovies(x-1);//moves down list of movies
			for (int i=0; i<currThree.size();i++) {
				System.out.println(currThree.get(i).getTitle());
			}
		}
	}
/**
 ...
 */
	public String [] runGenreMode(String []givenArr) {
		...
		while (!userInput.equals("x")) {
			...
			if(firstLetter=='s') {//selects genre
				arr[genre]="SELECTED";
				backend.addGenre(genreList.get(genre));
			}
			if(firstLetter=='u') {//unselects genre
				arr[genre]="UNSELECTED";
				backend.removeGenre(genreList.get(genre));
			}
		}
	...
	public String[] runRatingSelectionMode(String []givenArr) {
		...
		while (!userInput.equals("x")) {
			...
			if ((firstLetter!='s'&&firstLetter!='u')||rating<1||rating>10) {//check input validity
				System.out.println("Invalid Input");
				continue;
			}
	...
	private int turnStringToInt(String x) {
		...
		for (int i=0;i<x.length(); i++) {
			if ((int)x.charAt(i)>47&&(int)x.charAt(i)<58) 
				num+=(int)x.charAt(i)*10^(x.length()-i);
			else {
				return -1;
			}
		}
